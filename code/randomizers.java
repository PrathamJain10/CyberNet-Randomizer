...
public class randomizers {
    ...
    public static void main(String[] args) {
        ...
        try (BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(OUTPUT_FILE))) {
            ...
            int index = getRandomStartingIndex(remainingIndexes, random, 1); // Start from a random index between 1 and the last index
    ...
    private static int getNextControlledRandomAscendingIndex(int currentIndex, List<Integer> remainingIndexes, Random random) {
        ...
        // Select a random subset size to control randomness
        int subsetSize = Math.min(validIndexes.size(), random.nextInt(Math.max(1, validIndexes.size() / 2)) + 1);
        List<Integer> randomSubset = validIndexes.subList(0, subsetSize);
        return randomSubset.get(random.nextInt(randomSubset.size()));
    }

    private static int getRandomStartingIndex(List<Integer> remainingIndexes, Random random, int startIndex) {
        List<Integer> validIndexes = new ArrayList<>();
        for (Integer idx : remainingIndexes) {
            if (idx >= startIndex) {
                validIndexes.add(idx);
